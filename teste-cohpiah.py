...
def media_sentencas(sentencas): # retornando media menor do que esperado: dois possíveis problemas: somando menos caracteres ou dividindo por um número de sentenças maior do que o que existe;
    cont = 0
    totalChar = 0
    lista = []
    palavras = []
    for i in range(0, len(sentencas)):
        lista += separa_frases(sentencas[i])
    for j in range(0, len(lista)):
        palavras += separa_palavras(lista[j])
    for k in range(0, len(palavras)):
        totalChar += len(palavras[k]) # .!? ,:;
        if "..." in palavras[k]:
            cont = 1
        if "." in palavras[k]:
            totalChar -= 1
    ...
    if cont = 1:        
        ...
        mediaSentencas = totalChar / (len(sentencas)-1)
